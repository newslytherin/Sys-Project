...
class ApiFacade extends React.Component {
  ...
  newOrder = async (trip, id) => {
    try {
      fetch(
        this.getOrderTripURL(id),
        this.makeOptions("PUT", false, trip)
  ...
  editUser = async (user, id) => {
    try {
      fetch(this.getEditUserUrl(id), this.makeOptions("PUT", false, user)).then(
        handleHttpErrors
      );
    } catch (err) {
      console.log(`err:: ${err}`);
  ...
  addFlight = flight => {
    const options = this.makeOptions("POST", true, flight);
    return fetch(this.getAddFlightUrl(), options, true)
      .then(handleHttpErrors)
      .then(res => {
        return res;
      });
  ...
  signup = user => {
    ...
    return fetch(this.getUserSignupUrl(), options, true).then(res =>
  ...
  getUserOrders = () => {
    ...
    return fetch(this.getUserOrdersUrl() + this.getId(), options, true).then(
