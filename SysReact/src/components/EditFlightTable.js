...
import { HashRouter as Router, Route, NavLink, withRouter, Redirect } from "react-router-dom";
  ...
  return (
    <div className="order-expanded">
      ...
      <div className="info">
        ...
        {facade.loggedIn() && (
          <button onClick={e => editFlight(e, props.data, props.toggleEdit, props.setEditId)}>edit</button>
        )}
      </div>
    </div>
  );
...
function editFlight(e, data, toggleEdit, setEditId) {
  e.preventDefault();
...
async function getFlightsFrommDB() {
  let f = await facade.getOwnFligths();
...
export default function FilghtsTable() {
  ...
  const getTripFromId = () => {
    const item = data.filter((item) => item.id === editId)
    console.log(item[0])
    return item[0]
  }
  ...
  return (
    <>
    { isEdit && (<EditTrip toggleEdit={toggleEdit} getTripFromId={getTripFromId}/>)}
    { !isEdit && (<div className="main">
      ...
      {sortedData.map((item, i) => (
        <Trip key={i} data={item} toggleEdit={toggleEdit} setEditId={setEditId}/>
      ))}
